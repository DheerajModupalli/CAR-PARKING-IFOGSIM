...
public class carParkingcloud {
	static List<FogDevice> cloudDevices = new ArrayList<FogDevice>();
	...
	public static void main(String[] args) {
		Log.printLine("Starting simulation with cloud...");
		try {
			Log.disable();
			...
			for(FogDevice device : cloudDevices){
			...
			controller = new Controller("master-controller", cloudDevices, sensors, 
					actuators);
			
			controller.submitApplication(application, 
					(CLOUD)?(new ModulePlacementMapping(cloudDevices, application, moduleMapping))
							:(new ModulePlacementEdgewards(cloudDevices, sensors, actuators, application, moduleMapping)));
	...
	private static void createFogDevices(int userId, String appId) {
		...
		cloud.setParentId(-1);
		cloudDevices.add(cloud);
		FogDevice proxy = createFogDevice("proxy-server", 2800, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333);
		proxy.setParentId(cloud.getId());
		proxy.setUplinkLatency(100); // latency of connection between proxy server and cloud is 100 ms
		cloudDevices.add(proxy);
	...
	private static FogDevice addArea(String id, int userId, String appId, int parentId){
		...
		cloudDevices.add(router);
		router.setUplinkLatency(2); // latency of connection between router and proxy server is 2 ms
		for(int i=0;i<numOfCamerasPerArea1+10;i++){
